...
/**Function to get a templateId variables.
 * @param {string} templateId The string
 */
async function getTemplateData(templateId) {
    const templateData = await fetch("https://esms.dialog.lk/api/v1/message-template/view-template", {
        "credentials": "include",
        "headers": {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; rv:109.0) Gecko/20100101 Firefox/115.0",
            "Accept": "application/json, text/plain, */*",
        ...
        "referrer": "https://esms.dialog.lk/template/view-list/2409",
    ...
    if (!templateData.ok) {
        throw new Error(`HTTP error!  Status: ${templateData.status}`);
    }
    return new Promise ((resolve, reject) => {
        if (data.status === 'success') {
            resolve(data.data);
        } else {
            reject(`Error get template!  Status: ${data.comment}`);
        }
    })
}
/**
 * Function to get report
 * @param {Date} fromDate Report starting date
 * @param {Date} toDate report end date
 * @param {option} option daily or monthly
 */
async function getReport (fromDate, toDate, option) {
    if (option === 'daily') {
        const report = await fetch("https://esms.dialog.lk/api/v1/filter/date-wise/bulk/summary-report?currentPage=1&postsPerPage=5", {
            "credentials": "include",
            "headers": {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; rv:109.0) Gecko/20100101 Firefox/115.0",
                "Accept": "application/json, text/plain, */*",
            ...
            "referrer": "https://esms.dialog.lk/account/report/per-mobile",
        ...
        return new Promise ((resolve, reject) => {
            if(data.status === 'success'){
                ...
                reject(`Error getting report!  Status: ${data.comment}`);
            }
        })
    } if (option === 'monthly') {
        const dataReport = await fetch("https://esms.dialog.lk/api/v1/filter/month-wise/bulk/summary-report?currentPage=1&postsPerPage=5", {
            "credentials": "include",
            "headers": {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; rv:109.0) Gecko/20100101 Firefox/115.0",
                "Accept": "application/json, text/plain, */*",
            ...
            "referrer": "https://esms.dialog.lk/account/report/per-mobile",
            "body": `{\"fromMonth\":\"${fromDate}\",\"toMonth\":\"${toDate}\"}`,
            "method": "POST",
            "mode": "cors"
        });
        if(!dataReport.ok){
            throw new Error(`HTTP error!  Status: ${dataReport.status}`);
        }
        const reportData = await dataReport.json();
        return new Promise((resolve, reject) => {
            if(reportData.status === 'success'){
                resolve(reportData.userData);
            } else {
                reject(`Error getting report!  Status: ${reportData.status}`);
            }
        });
    }    
}
